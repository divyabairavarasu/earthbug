...
- Name: results-view.spec.js >> Results View >> KNOWN-BUG: missing name in result renders img alt as "undefined"
- Location: tests/results-view.spec.js:185:3
...
Locator: getByText(/missing required fields/i)
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for getByText(/missing required fields/i)
...
```ts
  ...
  198 |       p.waitForEvent('filechooser'),
  199 |       p.getByRole('button', { name: /upload photo/i }).click(),
  200 |     ]);
  201 |     await chooser.setFiles(ensureTestPng());
  202 |     // After security fix: missing 'name' is rejected as an invalid response
> 203 |     await expect(p.getByText(/missing required fields/i)).toBeVisible({ timeout: 15_000 });
      |                                                           ^ Error: expect(locator).toBeVisible() failed
  204 |     await ctx.close();
  205 |   });
  206 | });
